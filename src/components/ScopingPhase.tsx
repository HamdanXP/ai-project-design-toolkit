...
export const ScopingPhase = ({
  ...
  useEffect(() => {
    ...
    // Set the initial progress only if the phase isn't already completed
    if (currentPhaseStatus !== "completed") {
      const storedDecision = readyToAdvance === true ? 'proceed' : readyToAdvance === false ? 'revise' : null;
      ...
      if (storedDecision === 'proceed') {
        ...
        // Default progress calculation (0 initially)
        onUpdateProgress(0, totalSteps);
      }
    }
  }, [onUpdateProgress, updatePhaseStatus, currentPhaseStatus, readyToAdvance]);
  ...
  // CRITICAL: Update progress for steps 1-4 based on activeStep
  useEffect(() => {
    if (currentPhaseStatus !== "completed") {
      // Don't update progress for step 5, it's controlled by FinalFeasibilityGate buttons
      if (activeStep <= 4) {
  ...
  const moveToNextStep = () => {
    if (activeStep < totalSteps) {
      ...
      // Important: For step 5, we update the progress to 4/5 = 80%
      if (nextStep === 5) {
        onUpdateProgress(4, totalSteps);
      } else if (nextStep < 5) {
        // For steps 1-4, use normal progression
  ...
  const moveToPreviousStep = () => {
    if (activeStep > 1) {
      ...
      // Update progress normally for steps 1-4
      if (prevStep < 5) {
        onUpdateProgress(prevStep - 1, totalSteps); 
      }
      // For step 5, we maintain the current progress state based on readyToAdvance
    }
  };
