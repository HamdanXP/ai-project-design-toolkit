...
const ScopingPhasePage = () => {
  ...
  useEffect(() => {
    ...
    if (storedPhases) {
      ...
      const reflectionPhase = parsedPhases.find((p: ProjectPhase) => p.id === "reflection");
      setCanAccessPhase(reflectionPhase?.status === "completed");
    }
  }, []);
  ...
  const handleCompletePhase = () => {
    // Mark the scoping phase as completed
    setPhases(prevPhases => {
      const updatedPhases = prevPhases.map(phase => {
        if (phase.id === "scoping") {
          return {
    ...
    localStorage.setItem(`project_phase_scoping`, JSON.stringify(phaseData));
    ...
    navigate('/project/phases/development');
  };
        ...
        <div className="flex-1 flex flex-col pt-16">
          <div className="flex-1 p-4 md:p-6 overflow-y-auto">
            <div className="max-w-4xl mx-auto">
              <PhaseNavigation currentPhase="scoping" phases={phases} />
              
              <Card className="my-8">
                <CardContent className="p-8 flex flex-col items-center text-center">
                  <h2 className="text-2xl font-bold mb-4">Scoping Phase Locked</h2>
                  <p className="text-muted-foreground mb-6">
                    You need to complete the Reflection phase before accessing the Scoping phase.
                  </p>
                  <ArrowRight className="h-10 w-10 text-muted-foreground mb-4" />
                  <Button 
                    className="mt-2" 
                    onClick={() => navigate('/project/phases/reflection')}
                  >
...
export default ScopingPhasePage;
